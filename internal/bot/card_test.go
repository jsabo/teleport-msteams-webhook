...
func TestBuildCard_pending(t *testing.T) {
	...
	msg := BuildCard("req-123", data, mustURL(t, "https://proxy.example.com"), "teleport.example.com", "")
	j := cardJSON(t, msg)

	for _, want := range []string{
		"req-123", "teleport.example.com", "alice", "database-admin, dev",
		"P1 incident", "⏳ PENDING", "New Access Request", "/web/requests/req-123",
	} {
...
func TestBuildCard_emptyReason_omitted(t *testing.T) {
	...
	if strings.Contains(j, "\"Reason\"") {
		t.Errorf("expected Reason block to be omitted when empty\n%s", j)
	}
}

func TestBuildCard_clusterOmittedWhenEmpty(t *testing.T) {
	data := RequestData{User: "charlie", Roles: []string{"viewer"}}
	msg := BuildCard("req-000", data, nil, "", "")
	j := cardJSON(t, msg)

	if strings.Contains(j, "\"Cluster\"") {
		t.Errorf("expected Cluster fact omitted when clusterName is empty\n%s", j)
	}
}
